...
public class RoomsService : IRoomsService, IRoomManagerService
{
    ...
    public void SelectActiveStage(Guid roomId, Guid stageId)
    {
        ...
        if (stage == null) throw new StageNotFoundException(stageId);
    ...
    {
        if (!VoteOption.VoteOptions.Contains(vote)) throw new ArgumentException();
        var room = _roomStorage.GetRoom(roomId);
        if (room == null) throw new RoomNotFoundException(roomId);
    ...
    public IEnumerable<PlanningParticipant> GetRoomParticipants(Guid roomId)
    {
        ...
        return room.Participants ?? new List<PlanningParticipant>();
    }
    ...
    public async Task<RoomToken> CreateRoom(string displayName, string? roomPassword, bool useJira, string? jiraAddress)
    {
        ...
        var planningParticipant =
            new PlanningParticipant(Guid.NewGuid(), displayName, MemberType.Administrator, room.Id);
    ...
    public async Task<RoomToken> JoinRoom(string displayName, string? roomPassword, Guid roomId)
    {
        ...
        if (room.UsePassword)
        {
            if (roomPassword == null) throw new IncorrectPasswordException();
            var passwordHash = await CreateHash(roomPassword);
            var isPasswordCorrect = passwordHash.SequenceEqual(room.PasswordHash);
            if (!isPasswordCorrect) throw new IncorrectPasswordException();
        }

        if (room.Participants.Any(x => x.DisplayName == displayName)) throw new NameAlreadyTakenException(displayName);
